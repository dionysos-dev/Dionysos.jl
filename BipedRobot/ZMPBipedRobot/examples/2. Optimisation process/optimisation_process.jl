...
i = 1; 
solutions = Array[]
Niterations = length(Δz_space) * length(Tstep_space) *  length(Lmax_space) * length(δ_space)* length(δver_space) * length(hstep_space)
            ...
            for δ in δ_space
                for δver in δver_space
                    ...
                    for hstep in hstep_space 
                        ...
                        if (x[3] <=  sqrt(-(x[1])*(x[1] - L1 - L2))) && (x[2] * x[4] > 2*Ts)
...
plt_objective = plot(   #title = "Objective function plot for each candidate",
                        xlabel = "max $(L"f_1(x)")", ylabel = "min $(L"f_2(x)")", zlabel ="min $(L"f_3(x)")",
                        camera = (45,45), 
                        dpi=600,)
scatter!([objectives[:, 1]], [objectives[:, 2]], [objectives[ :, 2]], label = false, ms = 2, msw = 1)
display(plt_objective)
savefig(plt_objective, saveFolder * "/sim_objectives.png")
