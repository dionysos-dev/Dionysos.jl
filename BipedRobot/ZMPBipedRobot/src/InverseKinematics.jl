...
Constructor
"""
function InverseKinematics(; br::BipedRobot, fp::FootPlanner, ct::CoMTrajectory, sf::SwingFootTrajectory, check::Bool = false)
    ...
    if (check)
        plt = plot( #title = "Inverse Kinematrics", 
                    ...
                    #xlim = (-0.1, 0.1), 
                    layout = (1, 2), dpi = 600)
        scatter!(plt[1], steplocalL_plot[1,:], steplocalL_plot[3,:], ms = 2, msw = 0.1, label = false, title = "Local Left Foot")
        scatter!(plt[2], steplocalR_plot[1,:], steplocalR_plot[3,:],  ms = 2, msw = 0.1, label = false, title = "Local Right Foot")
        display(plt)
    ...
    if (check)
        plt_θ = plot(   title = "Inverse Kinematrics", 
                        # xlims = (0, tend), 
                        #ylims = (0.28, 0.32),
        ...
        for (side_idx, side) in enumerate(["Left", "Right"])
            for (joint, name) in enumerate(["Leg", "Knee"])
                plot!(plt_θ[joint, side_idx], tplot, qref[:,  (2*joint - 1) + (side_idx - 1)], label = false, title = "$(side) $(name)")
...
"""
function computeGlobal2Local(br::BipedRobot, fp::FootPlanner, ct::CoMTrajectory, sf::SwingFootTrajectory)
    center = fp.center; 
    stepR = sf.stepR; stepL = sf.stepL;
    ...
    for stepNum = 1 : length(stepL)
        ...
        for k = 1 : length(hip_height[stepNum][1, :])
