...
Constructor 
"""
function RobotSimulator(;   fileName::String = "ZMP_bipedalRobot.urdf",
                            symbolic::Bool = false,
    ...
    robot = getMechanism(; fileName, symbolic, use_urdf, T, add_contact_points, add_flat_ground, add_gravity, contactmodel)
...
function default_contact_model()
    SoftContactModel(hunt_crossley_hertz(k = 500e2), ViscoelasticCoulombModel(0.8, 20e3, 100.))
end
...
"""
function getMechanism(; fileName::String = "ZMP_bipedalRobot.urdf",
                        symbolic::Bool = false,
                        use_urdf::Bool = !symbolic,
                        T::Type = symbolic ? Num : Float64,
                        add_contact_points = true,
    ...
    if use_urdf	
        ...
        if (add_gravity)
            mechanism.gravitational_acceleration =  FreeVector3D(root_frame(mechanism), 0, 0, -9.81)
    ...
    if !symbolic && add_contact_points && contactmodel !== nothing 
        ...
        for side in (:left, :right)
            ...
            for div in 2 : 2
                for sign in [-1, 1]
                    cp = Point3D(frame, sign * left_foot_length/div, sign * left_foot_width/div, -left_foot_height)	
...
"""
function set_nominal!(rs::RobotSimulator, vis::MechanismVisualizer, boom::VecOrMat, actuators::VecOrMat, foot::VecOrMat)
    # boom = [ 0, 0 ]
    # foot = [ 0, 0 ]
    # actuators = [ 0, 0, 0, 0 ]
...
function update_visulizer!(rs::RobotSimulator, vis::MechanismVisualizer)
    set_configuration!(vis, RigidBodyDynamics.configuration(rs.state))
end
...
"""
function trajectory_controller!(rs::RobotSimulator,
