...
function defineBipedRobot(wo::WalkingOptimization)
    ...
    Δz = wo.Δz; Tstep = wo.Tstep; Lmax = wo.Lmax;
    δ = wo.δ; Tver = wo.Tver; hstep = wo.hstep; 
    zc = wo.zc; previewTime = wo.previewTime;
...
function computeAutoDefineParameters!(wo::WalkingOptimization)
    ...
    rs = RobotSimulator(   fileName = "ZMP_2DBipedRobot.urdf",
                                    symbolic = false, 
...
"""
function EnergyObjectiveFunction(torque::VecOrMat{T}, omega::VecOrMat{T}, dt::T) where T<:Real
    energy = zeros(4)
    power = zeros(4, length(torque[1, :]))
    for actuator in range(1, 4)
        energy[actuator], power[actuator, :] = computeEnergy(torque[actuator, :], omega[:, actuator], dt)
        # println("Energy $(actuator - dof_offset) = $(E[actuator - dof_offset]) J")
    end 
    total_energy = sum(energy)
    total_power = sum(power[actuator, :] for actuator = 1 : 4)
...
"""
function simulate(rs::RobotSimulator, qref::Matrix, tref::Union{Vector, StepRangeLen},  tend::Float64, Kp::Float64, Ki::Float64, Kd::Float64, Δt::Float64)
    ctrl = true 
    boom = [ 0, 0]
    foot = [ 0, 0 ]
    actuators = [ 0, 0, 0, 0 ]
...
function ZMPbipedObjective(x)
    ...
    wo = WalkingOptimization(   Δz, Tstep, Lmax, δ, Tver, hstep,
                                zc, previewTime,
                                NaN, NaN )
    ...
    tend_sim = tplot[end]
    tsim, qsim, vsim, torque_sim, CoMsim = simulate(rs, qref, tplot, tend_sim, Kp, Ki, Kd, 1e-3)
    ...
    tau_constraints = (maximum(abs.(torque_sim[3, :]))  < tau_hip_max) && (maximum(abs.(torque_sim[4, :])) < tau_hip_max) &&  (maximum(abs.(torque_sim[5, :]))  < tau_knee_max) && (maximum(abs.(torque_sim[6, :])) <  tau_knee_max) 
    omega_constraints = (maximum(abs.(vsim[:, 3]))  < omega_hip_max) && (maximum(abs.(vsim[:, 4])) < omega_hip_max) &&  (maximum(abs.(vsim[:, 5]))  < omega_knee_max) && (maximum(abs.(vsim[:, 6])) <  omega_knee_max) 
    ...
    if (tau_constraints && omega_constraints)
        ...
        energyPerStep,_,_ = EnergyObjectiveFunction(torque_sim[3:6, i_start:i_end], vsim[i_start:i_end, 3:6], Δt)
    ...
    g =  [x[3] - sqrt(2*L2^2 * (1-cos( acos(((L1 + L2 - x[1])^2 - L1^2 - L2^2)/(2*L1*L2)))))]
