...
function PreviewController(;br::BipedRobot, check::Bool = false)
    ...
    if(check)
        Ts = br.Ts
        plt = plot( title = "Preview Controller", 
                    xlabel = "Preview time [s]", ylabel = "Preview Gain -Gd",
                    dpi = 600)
...
function computeGains(br::BipedRobot)
    ...
    while (norm(Ka[2]-Ka[1], 2) > ϵ) 
        Ka[1] = Ka[2];
        Ka[2] = transpose(Aa) * Ka[1] * Aa - transpose(Aa) * Ka[1]  * Ba* (R[] + transpose(Ba)  *  Ka[1]  * Ba)^-1 * transpose(Ba)  * Ka[1] * Aa  + Qa;
    end
