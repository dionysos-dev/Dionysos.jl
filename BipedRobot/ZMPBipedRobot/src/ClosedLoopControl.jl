...
function oneStepPC!(br::BipedRobot, pc::PreviewController, zt::ZMPTrajectory, CLc::CLcontrol)
    Ad = br.Ad; Bd = br.Bd; Cd = br.Cd; 
    Ts = br.Ts; previewTime = br.previewTime;
    ZMP = zt.ZMP; zc = br.zc; g = br.g;
    ...
    CLc.previous_state = xx_next; 
end 

function ClosedLoopZMPcontroller!(br::BipedRobot, rs::RobotSimulator,
                                Δt::Float64,
                                Kp::Float64, Ki::Float64, Kd::Float64,
                                debugMode::Bool=false)

    time_idx = 1; stepNum = 1; 
...
function measureZMP(    rs::RobotSimulator,
    ...
    pr_x_right = (-contact_torque_right[2] - contact_force_right[1] * d)/contact_force_right[3]
    pr_y_right = (contact_torque_right[1] - contact_force_right[2] * d)/contact_force_right[3]

    pr_x_left = (-contact_torque_left[2]- contact_force_left[1] * d)/contact_force_left[3]
    pr_y_left = (contact_torque_left[1]- contact_force_left[2] * d)/contact_force_left[3]

    px = (pr_x_right * contact_force_right[3] + pr_x_left * contact_force_left[3])/(contact_force_right[3] + contact_force_left[3])
    py = (pr_y_right * contact_force_right[3] + pr_y_left * contact_force_left[3])/(contact_force_right[3] + contact_force_left[3])
