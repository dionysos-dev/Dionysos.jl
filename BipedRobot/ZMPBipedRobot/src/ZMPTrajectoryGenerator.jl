...
function ZMPTrajectory(;br::BipedRobot, fp::FootPlanner, check::Bool = false)
    ...
    if(check)
        ...
        # plt = plot( title = "ZMP Trajectory", 
        #             xlabel = "time [s]",
        #             layout = (2,1))
        # i = 1
        # for dir in ["X[m]", "Y[m]"]
        #     plot!(plt[i], tplot, ZMPplot[i, :], ylabel = dir, label = false)
        #     i = i + 1;
        # end 
        plt = plot( title = "ZMP Trajectory", 
        ...
        scatter!(getindex.(center, 1), getindex.(center,2), label = "Center", mc =:black, markershape=:xcross)
...
function computeZMPTrajectory(br::BipedRobot, fp::FootPlanner) 
    ...
    isLeftSupport = br.isLeftSupport; δ = br.δ; 
    Tstep = br.Tstep; Ts = br.Ts; Tdelay = br.Tdelay; Twait = br.Twait; 
    right = fp.right;
    left = fp.left;
    center = fp.center;
