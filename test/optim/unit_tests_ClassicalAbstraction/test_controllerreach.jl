...
@testset "ControllerReach" begin
    ...
    AB.ClassicalAbstraction.compute_controller_reach!(
        contr,
        symmodel.autom,
        initlist,
        targetlist,
    ...
    if VERSION >= v"1.5"
        function f(autom, initlist, targetlist)
            ...
            initset, targetset, num_targets_unreachable, current_targets, next_targets =
                AB.ClassicalAbstraction._data(contr, autom, initlist, targetlist)
            ...
            @allocated AB.ClassicalAbstraction._compute_controller_reach!(
                contr,
                autom,
                initset,
                targetset,
