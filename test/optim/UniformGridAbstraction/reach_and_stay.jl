...
@testset "ControllerReachAndStay" begin
    ...
    _I_ = LazySets.Hyperrectangle(; low = SVector(-1.9), high = SVector(-1.7))   # start near the left edge
    _T_ = LazySets.Hyperrectangle(; low = SVector(-0.4), high = SVector(0.4))    # target around the origin
...
@testset "ControllerReachAndStay: stay_on_first_entry" begin
    ...
    _I_ = LazySets.Hyperrectangle(; low = SVector(-1.9), high = SVector(-1.7))
    _T_ = LazySets.Hyperrectangle(; low = SVector(-0.4), high = SVector(0.4))
