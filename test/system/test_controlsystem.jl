...
@testset "buildAffineApproximation" begin
    ...
    X = IntervalBox(-10.0..10.0); U = IntervalBox(-10.0..10.0); W = IntervalBox(-10.0..10.0)
    ...
    @test approx_sys.U === U
    @test approx_sys.W === W
    @test isapprox(MathematicalSystems.successor(approx_sys, [0.], [0.], [0.]), [0.], atol = TOL)
end
...
@testset "AffineApproximationDiscreteSystem" begin
    ...
    U = [(-Inf, -Inf), (Inf, Inf)]
    W = [(-Inf, -Inf), (Inf, Inf)]
    sys = MathematicalSystems.NoisyConstrainedAffineControlDiscreteSystem(A, B, c, D, X, U, W)
    L = 1. # Just for the example, not mathematically correct

    obj_from_sys = ST.AffineApproximationDiscreteSystem(sys, L)
    @test obj_from_sys.constrainedAffineSys === sys
    @test obj_from_sys.L === L
    @test isapprox((obj_from_sys.f_eval)([0., 0.], [0., 0.], [0., 0.]), c, atol = TOL)
    ...
    @test isapprox((obj_from_matrices.f_eval)([0., 0.], [0., 0.], [0., 0.]), c, atol = TOL)
end
