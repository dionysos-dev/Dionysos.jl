...
@testset "BDD.BitSet" begin
    set = BDD.BitSet()
    @test isempty(set)
    @test sprint(show, MIME"text/plain"(), set) == "Main.Dionysos.Utils.BDD.BitSet with 0 bits"
    ...
    @test isempty(empty!(set))
    @test isempty(set)
    @test sprint(show, MIME"text/plain"(), set) == "Main.Dionysos.Utils.BDD.BitSet with 5 bits"
    push!(set, 40)
    @test sprint(show, MIME"text/plain"(), set) in [
        "Main.Dionysos.Utils.BDD.BitSet with 6 bits",       # Julia v1.0
        "Main.Dionysos.Utils.BDD.BitSet with 6 bits:\n  40" # Julia v1.5
    ]
end
