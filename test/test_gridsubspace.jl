...
AB.add_coord!(gridspace, SVector(1.2, 3.5))
@test AB.get_ncells(gridspace) == 1
AB.add_coord!(gridspace, SVector(-0.5002, -1.0))
@test AB.get_ncells(gridspace) == 2

AB.add_set!(gridspace, AB.HyperRectangle(SVector(1.0, 0.0), SVector(11.0, 10.0)), AB.OUTER)
@test AB.get_ncells(gridspace) == 67

AB.remove_coord!(gridspace, (2.0, 2.0))
@test AB.get_ncells(gridspace) == 66
AB.remove_set!(gridspace, AB.HyperRectangle(SVector(5.0, 5.0), SVector(10000.0, 10000.0)), AB.INNER)
...
AB.remove_set!(subset, AB.HyperRectangle(SVector(1.0, 1.0), SVector(2.0, 2.0)), AB.OUTER)
@test AB.get_ncells(subset) == 45
AB.add_set!(subset, AB.HyperRectangle(SVector(0.0, 0.0), SVector(5.0, 5.0)), AB.INNER)
