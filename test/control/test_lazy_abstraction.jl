...
function test()
    ...
    time_controller =
        @elapsed AB.compute_controller_reach!(contr, symmodel.autom, initlist, targetlist)
    println("time for abstraction: ", time_abstraction)
    println("time for controller: ", time_controller)
    return println("total time: ", time_abstraction + time_controller)
end
