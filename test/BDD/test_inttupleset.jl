...
@testset "IntTupleSet" begin
    ...
    @test isempty(set)
    @test sprint(show, MIME"text/plain"(), set) == "Main.BDD.IntTupleSet{3,Int16} with 0×0×0 bits"
    ...
    @test isempty(empty!(set))
    @test isempty(set)
    @test sprint(show, MIME"text/plain"(), set) == "Main.BDD.IntTupleSet{3,Int16} with 3×4×4 bits"
    @test push!(set, Int16.((40, 1, 1))) === set
    @test sprint(show, MIME"text/plain"(), set) in [
        "Main.BDD.IntTupleSet{3,Int16} with 6×4×4 bits", # Julia v1.0
        "Main.BDD.IntTupleSet{3,Int16} with 6×4×4 bits:\n  (40, 1, 1)" # Julia v1.5
    ]
end
