...
function unicycleRobot()
    ...
    f = [
        px + T * v * mysinc(T * ω / 2) * cos(th + T * ω / 2)
...
finv(x, x₊) = begin
    ω = (x₊[3] - x[3]) / Ts
    [
        0.5 * (x₊[1] - x[1]) / (Ts * sinc(Ts * ω / 2 / pi) * cos(x[3] + Ts * ω / 2)) +
        0.5 * (x₊[2] - x[2]) / (Ts * sinc(Ts * ω / 2 / pi) * sin(x[3] + Ts * ω / 2))
        ω
    ]
end
# sys dimensions
...
function sample_x(; probSkew = 0.0, probX0 = 0.05)
    ...
    elseif randVal > probSkew
        ...
        r = dist / intialDist
        return (X0.c * l + closestNode.state.c * (1 - l)) * (1 - 0.3 * r) +
...
while !(X0 ∈ Xnew) && maxIter > 0
    ...
    closeNodes = first(
        sort(
            findCloseNodes(treeLeaves, xsample; d = Inf);
            lt = (
                (e1, e2) ->
                    UT.pointCenterDistance(e1.state, xsample) <
                    UT.pointCenterDistance(e2.state, xsample)
            ),
        ),
        5,
    )
    ...
    for Xclosest in closeNodes
        ...
        (sys, L) = Dionysos.System.buildAffineApproximation(
            fT,
            x,
            u,
            w,
        ...
        # println(L)
        El, kappa, cost = Dionysos.Symbolic.hasTransition(
            xnew,
            unew,
            Xclosest.state,
            sys,
    ...
    if ElMin !== nothing
        ...
        push!(
            treeLeaves,
            UT.Node(Xnew; parent = parentMin, action = kappaMin, path_cost = minPathCost),
        )
...
if true
    ...
    function projectEllipsoid(elli::UT.Ellipsoid)
        ...
        return UT.Ellipsoid(P, elli.c[1:2])
    end
    ...
    function plotEllipsoid!(ax, elli; ngrid = 25, alpha = 1.0, color = "blue")
        ...
        Lm = inv(L)
        xR = [
            [l[1] * x + l[2] * y + l[3] * z + c for (x, y, z) in Iterators.zip(x, y, z)] for (l, c) in Iterators.zip(eachrow(Lm), center)
        ]
        return ax.plot_surface(xR[1], xR[2], xR[3]; alpha = alpha, color = color)
    end
    ...
    function plotEllipse!(
        ...
        return ax.add_collection(polylist)
    end
    plotted = []
    ...
    for n in treeLeaves
        ...
        plotEllipse!(
            ax,
            projectEllipsoid(n.state);
            fc = (0.4 * lyap / LyapMax + 0.5, 0.4 * (LyapMax - lyap) / LyapMax + 0.5, 0.75),
            ew = 0.5,
        )
        nodePar = n.parent
        aTail = n.state.c
        while nodePar !== nothing
            ...
            if !(nodePar ∈ plotted)
                ...
                plotEllipse!(
                    ax,
                    projectEllipsoid(nodePar.state);
                    fc = (
                        0.4 * lyap / LyapMax + 0.5,
                        0.4 * (LyapMax - lyap) / LyapMax + 0.5,
                        0.75,
                    ),
                    ew = 0.5,
                )
    ...
    while currNode !== nothing
        ...
        plotEllipse!(
            ax,
            projectEllipsoid(currNode.state);
            fc = (0.4 * lyap / LyapMax + 0.5, 0.4 * (LyapMax - lyap) / LyapMax + 0.5, 0.75),
            ew = 2,
        )
        global currNode = currNode.parent
    ...
    PyPlot.colorbar(
        PyPlot.ScalarMappable(;
            norm = PyPlot.cm.colors.Normalize(; vmin = 0, vmax = LyapMax),
            cmap = cmap,
        );
        shrink = 0.7,
    )
