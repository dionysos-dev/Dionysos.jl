...
obstacles = [
    UT.Ellipsoid(Matrix{Float64}(I(nx)) * 1 / 50, [0.0; 0.0]),
    UT.Ellipsoid(Matrix{Float64}([2.0 0.2; 0.2 0.5]) * 1 / 2.0, [12.0; 0.0]),
    UT.Ellipsoid(Matrix{Float64}([0.5 0.2; 0.2 2.0]) * 1 / 2.0, [20.0; 0.0]),
    UT.Ellipsoid(Matrix{Float64}([0.5 0.2; 0.2 2.0]) * 1 / 6.0, [10.0; -10.0]),
]
...
function stop_crit(tree, LNnew, EF, EI, distance, optimizer; continues = true)
    problem = optimizer.problem
    minDist = 10.0
    for Nnew in LNnew
        E = Nnew.state
