...
function path_planning(frame_length; nsteps = nothing,
    ...
    target = AB.HyperRectangle(SVector(frame_length - 1.0, 0.5, -100.0), SVector(frame_length - 0.4, 0.8, 100.0))
    x0 = SVector(0.0, 0.0, 0.0)

    Xgrid = AB.NewGridSpaceList(x0, h)
    AB.add_set!(Xgrid, frame, AB.OUTER)
    for (x1lb, x2lb, x1ub, x2ub) in zip(X1_lb, X2_lb, X1_ub, X2_ub)
        box = AB.HyperRectangle(SVector(x1lb, x2lb, frame.lb[3]), SVector(x1ub, x2ub, frame.ub[3]))
