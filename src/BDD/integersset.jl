"""
    mutable struct IntegersSet <: AbstractSet{NTuple{N,Int} where N}

Same as `Base.Set{NTuple{N,Int} where N}` but with `CUDD`.
"""
mutable struct IntegersSet{N,T<:Integer} <: AbstractSet{NTuple{N,T}}
...
function IntegersSet{N,T}() where {N,T}
    ...
    return IntegersSet{N,T}(manager, variables, indexes, root, phase_, vars_, z_)
end

IntegersSet{N}() where N = IntegersSet{N,Int}()

Base.show(io::IO, set::IntegersSet) = Base.summary(io, set)
function Base.summary(io::IO, set::IntegersSet)
...
Base.eltype(::Type{IntegersSet{N,T}}) where {N,T} = NTuple{N,T}
Base.empty(::IntegersSet{N}, ::Type{T}=Int) where {N,T} = IntegersSet{N,T}()
Base.emptymutable(::IntegersSet{N}, ::Type{T}=Int) where {N,T} = IntegersSet{N,T}()
Base.isempty(set::IntegersSet) = set.root === CUDD.Cudd_ReadLogicZero(set.manager)
function Base.empty!(set::IntegersSet)
    set.root = CUDD.Cudd_ReadLogicZero(set.manager)
    return set
end
Base.IteratorSize(::IntegersSet) = Base.SizeUnknown()

# `x` will be used to denote a tuple of `Int`, and `e` to denote its elements
...
function Base.push!(set::IntegersSet{N,T}, x::NTuple{N,T}) where {N,T}
    _phase!(set, x)
...
function Base.in(x::NTuple{N,T}, set::IntegersSet{N,T}) where {N,T}
...
Base.iterate(set::IntegersSet{N,T}) where {N,T} = iterate(set, ntuple(i -> zero(T), Val(N)))

function Base.iterate(set::IntegersSet{N}, state::NTuple{N}) where N
