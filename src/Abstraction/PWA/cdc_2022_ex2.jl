...
function transFdict(transitionCost)
    testgraph = Vector{Tuple{String, String, Float64}}()
    for (key, value) in transitionCost
        # append!(testgraph,(string(key(1)),string(key(2)),value))
        push!(testgraph,(string(key[1]),string(key[2]),value))
    end
    return testgraph
end
...
# control design
src, dst = "232", "153" # initial and goal sets
gc = AB.Digraph(testgraph) 
path, cost = AB.dijkstrapath(gc, src, dst) # gets optimal path
...
for l = 1:length(path)-1
    new_action = (parse(Int64, path[l]), parse(Int64, path[l+1]))
