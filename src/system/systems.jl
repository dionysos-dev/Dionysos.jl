...
struct VectorContinuousSystem{T <: MS.AbstractContinuousSystem} <:
       MS.AbstractContinuousSystem
    systems::Vector{T}
end

MS.stateset(sys::VectorContinuousSystem) = Tuple(MS.stateset(s) for s in sys.systems)

MS.inputset(sys::VectorContinuousSystem) = Tuple(MS.inputset(s) for s in sys.systems)
