...
function replace_occurrence!(file_path::AbstractString, old::AbstractString, new::AbstractString, k)
    ...
    old_len = length(old)
    new_string = string(file_string[1:start_idx-1], new, file_string[start_idx+old_len:end])
...
# for occurence k
function write_after_string!(file::AbstractString, str_before::AbstractString, str_after::AbstractString, k)
    replace_occurrence!(file, str_before, str_before*str_after, k) 
end