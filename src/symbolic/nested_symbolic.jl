...
function NewNestedSymbolicModel(dom::DO.GeneralDomainList, Udom, param)
    ...
    return symmodel = NestedSymbolicModel(
        Xdom,
        Udom,
...
function get_state_by_xpos(symmodel::NestedSymbolicModel, pos, l)
...
function delete_state!(symmodel::NestedSymbolicModel, s::Int)
    ...
    return symmodel.active[s] = false
end
...
function delete_transitions_post!(symmodel, source, symbol)
    return delete_transition_post!(symmodel.autom, source, symbol)
end
...
function split_node!(symmodel, sys, s)
    ...
    return update_outgoing_transitions_MC!(symmodel, sys, s, subcells)
end
...
function Plots.plot!(symmodel::NestedSymbolicModel; dims = [1, 2], annotate = false)
    ...
    for value in symmodel.xint2pos
        ...
        plot!(grid, pos; opacity = 0.9, color = colors)
    end
end

function circleShape(h, k, r)
    angle = LinRange(0, 2 * pi, 500)
    return h .+ r * sin.(angle), k .+ r * cos.(angle)
end
        ...
        for (source, symbol, proba) in get_transitions_pre(symmodel, target)
            if !bool || source == src
                ...
                if source != target
                    ...
                    h = DO.get_h(gridx)
                    plot!(
                        circleShape(x[1] + h[1] / 12.0, x[2] + h[1] / 12.0, h[1] / 12.0);
                        seriestype = [:shape],
                        lw = 2.0,
                        c = :blue,
                        fillalpha = 0.0,
...
function plot_trajectory!(sys, x0, l; color = :red, dims = [1, 2])
    ...
    return scatter!([0.0], [0.0]; color = :red, markersize = 2)
end
...
function update_MC!(symmodel::NestedSymbolicModel)
    ...
    return symmodel.mc = build_Markov_Chain(symmodel)#build_Markov_Chain(symmodel)
end

function plot_shannon_entropy(
    sys,
    ...
    fig = plot(;
        aspect_ratio = 1,
        legend = false,
        title = "outgoing shannon-entropy",
        xlims = xlims,
        ylims = ylims,
    )
...
function plot_shannon_entropy!(
    ...
    for s in get_cells(symmodel)
        ...
        if abs(x[1]) < 10 && x[2] < 20 && x[2] > -8.0
            ...
            opacity = val / max_val
            if opacity > 0.01
                plot!(dom.grid, pos; opacity = opacity * 2.0, color = :yellow)
            end
        end
    end
end
...
function plot_steady_state!(
    ...
    for s in get_cells(symmodel)
        ...
        if opa > tol
            plot!(dom.grid, pos; opacity = opa * 12.0, color = color)
        end
    end
end
...
function plot_region!(symmodel, region)
    for s in keys(region)
        ...
        (l, pos) = get_xpos_by_state(symmodel, s)
        dom = symmodel.Xdom.domains[l]
        plot!(dom.grid, pos; opacity = 1.0, color = :yellow)
    end
end
...
function plot_Jacobian(sys, symmodel::NestedSymbolicModel, Jacobian; dims = [1, 2])
    ...
    for (i, value) in enumerate(symmodel.xint2pos)
        ...
        if r1 < 0 && r2 < 0
            plot!(grid, pos; opacity = 1.0, color = :green)
        elseif r1 > 0 && r2 > 0
            plot!(grid, pos; opacity = 1.0, color = :red)
        else
            plot!(grid, pos; opacity = 1.0, color = :yellow)
        end
    end
    x0 = SVector(2.0, 2.0)
    plot_trajectory!(sys, x0, 200)
