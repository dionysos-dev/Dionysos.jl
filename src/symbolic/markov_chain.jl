...
mutable struct MarkovChain
    ...
    symmodel_from_mc::Any #list
    mc_from_symmodel::Any #dictionnary
        ...
        if communications[2][i]
            ...
            for (j, s) in enumerate(class)
                ...
                if steady_state[s] > 0.0
                    ...
                    entropy_SS_finale[s] =
                        entropy_SS[s] + steady_state[s] * get_entropy_volume(symmodel, s)
                end
            end
...
function new_MarkovChain_Sparse(
    symmodel_from_mc,
    mc_from_symmodel,
    transition_matrix,
    transition_matrix_2,
