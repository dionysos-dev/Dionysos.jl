...
get_concrete_state(sym::SymbolicModel, state) =
    MP.get_coord_by_state(get_state_mapping(sym), state)
get_concrete_input(sym::SymbolicModel, input) =
    MP.get_coord_by_state(get_input_mapping(sym), input)
...
function get_states_from_set_strict(sym::SymbolicModel, set, incl_mode::MP.INCL_MODE)
