...
mutable struct LazyAbstraction{
    T,
    SM,
    C,
    TC <: Function,
...
function LazyAbstraction(
    ...
    pre_image,
    post_image;
    heuristic_data = nothing,
    transitions_previously_added = nothing,
)
    transitions_added =
        MutableMatrix(false, symmodel.autom.nsymbols, symmodel.autom.nstates)
    num_targets_unreachable =
        MutableMatrix(0, symmodel.autom.nsymbols, symmodel.autom.nstates)
    ...
    if transitions_previously_added === nothing
        transitions_previously_added =
            MutableMatrix(-1, symmodel.autom.nsymbols, symmodel.autom.nstates)
    end
    return LazyAbstraction{
        State,
        typeof(symmodel),
        typeof(contsys),
        typeof(transition_cost),
        typeof(pre_image),
        typeof(post_image),
        typeof(heuristic_data),
    }(
        initial,
        goal,
        symmodel,
...
function compute_controller(
    symmodel,
    contsys,
    initlist::Vector{Int},
    targetlist::Vector{Int},
    transition_cost,
    pre_image,
    post_image,
    h;
...
function plot_result!(problem; dims = [1, 2], x0 = nothing)
    ...
    for k in 1:(symmodel.autom.nstates)
