...
"""
struct OptimalControlProblem{N, T, C, U, TC, CA, F <: Tuple{Vararg{Function}}, E} <:
       BB.Abstract_BB_Problem
    # data of the problem
    x0::SVector{N, T}
    contsys::C
...
function OptimalControlProblem(
    ...
    q0, qT, coarse_abstraction, cells = P.Initialise(
        partition,
        contsys,
        Udom,
        _I_,
...
function build_medium_abstraction!(prob, cell)
    hx = prob.hx_medium
    Xdom = D.GeneralDomainList(
        hx;
        periodic = prob.periodic,
        periods = prob.periods,
        T0 = prob.T0,
    )
    ...
    problem = AS.symmodelProblem(
        symmodel,
        prob.contsys,
        prob.functions[1],
        prob.functions[2],
        AS.get_possible_transitions_2,
    )
    autom = AS.build_alternating_simulation(problem)
    symmodel = AB.with_automaton(symmodel, autom)
    return cell.medium_abstraction = symmodel
end
function build_heuristic!(cell, from)
    ...
    return heuristics[from] = heuristic
...
function BB.compute_lower_bound!(prob::OptimalControlProblem, node::BB.Node)
    ...
    return node.lower_bound = cost#+ 5 # where 1 is the cost of the transition
...
function build_fine_abstraction!(prob, cell)
    hx = prob.hx_fine
    Xdom = D.GeneralDomainList(
        D.build_grid_in_rec(cell.hyperrectangle, hx);
        periodic = prob.periodic,
        periods = prob.periods,
        T0 = prob.T0,
    )
    ...
    return cell.fine_abstraction = (symmodel, nothing)
end
function build_controller!(prob, cell, prev, next)
    ...
    problem, success = LA.compute_controller(
        symmodel,
        prob.contsys,
        initlist,
        targetlist,
        prob.transition_cost,
        prob.functions[4],
        prob.functions[3],
        h2;
...
function BB.expand(prob::OptimalControlProblem, node::BB.Node)
    ...
    for neighbor_cell in current_cell.outneighbors
        ...
        if prob.option[1] && (
            f1(reverse(new_path)) ||
            (length(path) >= 2 && neighbor_cell == path[end - 1]) ||
            f2(new_path)
        )
            continue
        end
...
function simulate_trajectory(prob::OptimalControlProblem, path)
    ...
    for (i, index) in enumerate(path)
        ...
        _T_L = cell.local_target_set[next]
        targetlist = U.get_symbols(symmodel, _T_L, AB.INNER)
        (local_traj, local_cost, succeed) = simulate_local_trajectory(
            prob.contsys,
            symmodel,
            contr,
            x0,
            targetlist;
            transition_cost = prob.transition_cost,
        )
...
function simulate_local_trajectory(
    contsys,
    symmodel,
    contr,
    x0,
...
function print_trajectory!(traj)
    ...
    return scatter!([x2[1]], [x2[2]]; color = :yellow, markersize = 3)
end
