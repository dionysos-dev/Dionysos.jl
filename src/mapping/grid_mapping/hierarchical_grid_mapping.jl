...
function HierarchicalGridMapping(m0::ImplicitGridMapping{N,T};
                                nlevels::Int=1, div::Int=2) where {N,T}
...
get_n_state(h::HierarchicalGridMapping) =
    h.offsets[end] + get_n_state(h.levels[end])
...
function _locate(h::HierarchicalGridMapping, q::Int)
    ...
    error("unreachable")  # offsets inconsistent
end
...
function get_state_by_pos(h::HierarchicalGridMapping{N}, l::Int, pos::NTuple{N,Int}) where {N}
...
function add_level!(h::HierarchicalGridMapping{N,T,ML}; div::Int=2) where {N,T,ML}
    last = h.levels[end]
    last isa ImplicitGridMapping || error("add_level! default only supports ImplicitGridMapping; provide a constructor for your mapping type")
