# "Set" assumes symmodel is empty initially... May fail if not respected
function compute_symmodel_from_controlsystem!(symmodel, contsys)
	...
	@showprogress 1 "Computing symbolic control system: " for u_rp in enumerategridspace_ref_pos(Ugrid)
		u = get_coord_by_pos(Ugrid, u_rp[2])
		r = Xgrid.h./2 .+ contsys.measnoise
		r = contsys.bound_map(r, u, contsys.tstep)
		r = r .+ contsys.measnoise
		for x_rp in enumerategridspace_ref_pos(Xgrid)
			x = get_coord_by_pos(Xgrid, x_rp[2])
			...
			if any(y_pos -> !has_pos(Xgrid, y_pos), ypos_iter)
				continue
			end
			add_to_symmodel_by_new_refs_coll!(symmodel, (x_rp[1], u_rp[1], get_ref_by_pos(Xgrid, y_pos)) for y_pos in ypos_iter)
