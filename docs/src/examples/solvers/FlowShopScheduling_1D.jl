...
for (i, t) in enumerate(HybridSystems.transitions(auto))
    ...
    if guard !== nothing
        ...
        for t in t_grid
            if t >= t_min && t <= t_max
                vline!([t], color=color_task, alpha=0.25, linewidth=1, linestyle=:dot, label=false)
            end
        end
        ...
        for x in x_grid
            if x >= x_min && x <= x_max
                hline!([x], color=color_task, alpha=0.25, linewidth=1, linestyle=:dot, label=false)
            end
        end
    end
end
...
plot!(t_vals, x_vals, color=:black, linewidth=2, label="Trajectory", linestyle=:solid, alpha=0.8)

## Initial point and switches
scatter!([t_vals[1]], [x_vals[1]], color=:green, marker=:star5, markersize=8, label="Initial state")
...
if !isempty(switch_indices)
    ...
    scatter!(t_switches, x_switches, color=:red, marker=:diamond, markersize=8, label="Switch")
    for i in switch_indices
        plot!([t_vals[i-1], t_vals[i]], [x_vals[i-1], x_vals[i]], color=:red, linewidth=3, linestyle=:dash, alpha=0.9, label=false)
    end
end
...
if !isempty(switch_indices)
    ...
    scatter!(t_starts, x_starts, color=:blue, marker=:circle, markersize=5, label="Task start")
end
...
for i in 1:step:length(ctrls)
    if i <= length(t_vals) && ctrls[i] != "switch" && ctrls[i] != "SWITCH"
        ...
        elseif isa(ctrls[i], Vector) && length(ctrls[i]) > 0
            control_values = join([Printf.@sprintf("%.2f", Float64(u)) for u in ctrls[i]], ", ")
