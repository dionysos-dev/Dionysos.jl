...
Base.@kwdef struct SimplePendulumMPPIConfig
    ...
    nstep::Int = 55
    input_values::Tuple{Vararg{Float64}} = Tuple(-2.1:0.25:2.1)

    terminal_radius::Float64 = 0.25
    use_terminal_inner_ellipsoid::Bool = true
    terminal_shrink::Float64 = 1.0
    terminal_outside_weight::Float64 = 1.0e6
    terminal_center_weight::Float64 = 0.0
    terminal_success_distance2::Float64 = 1.0
    λ::Float64 = 0.0005
    maxδx::Float64 = 180.0
    maxδu::Float64 = 140.0
    state_scaling_mode::Symbol = :matrix # :none, :std, :legacy, or :matrix
    state_scaling_std_floor::Float64 = 1.0e-3
    # In :matrix mode, only the diagonal is used as the coordinate scaling.
    state_scaling_matrix::Matrix{Float64} = Matrix{Float64}(LA.Diagonal([2.0, 15.0]))
    use_state_scaling::Bool = true
    state_scaling::Vector{Float64} = [2.0, 15.0]*2 # legacy scaling, used if state_scaling_mode == :legacy and use_state_scaling == true
    ...
    adaptive_box_search_scales::Vector{Float64} =
        [0.5, 0.7, 0.85, 1.0, 1.15, 1.35, 1.6, 2.0, 2.5, 3.0, 4.0, 5.0]
    ...
    planning_input_scale::Float64 = 0.6
    mppi_nsamples::Int = 2800
    mppi_niter::Int = 20
    mppi_λ::Float64 = 1.75
    mppi_noise_u::Float64 = 0.5
    rng_seed::Int = 11
    kappa_statistical_samples::Int = 300
    kappa_statistical_seed::Int = 11
end
...
function matrix_state_scaling(M)
    ...
    all(isfinite, A) || error("state_scaling_matrix entries must be finite")

    diag_scaling = abs.(LA.diag(A))
    all(x -> x > 0.0, diag_scaling) ||
        error("state_scaling_matrix diagonal entries must be nonzero")
    return diag_scaling
end

function resolve_state_scaling(cfg::SimplePendulumMPPIConfig, candidate)
    ...
    cfg.state_scaling_mode == :legacy &&
        return cfg.use_state_scaling ? Float64.(cfg.state_scaling) : nothing
    return error(
        "Unknown state_scaling_mode $(cfg.state_scaling_mode). Use :none, :std, :legacy, or :matrix.",
    )
end

function _build_simple_pendulum_certifier(
    problem,
    _system_cfg,
    _control_cfg,
    cfg::SimplePendulumMPPIConfig,
...
mutable struct SimplePendulumScalingCertifier <: SC.AbstractSymbolicCertifier
    problem::Any
    system_cfg::Any
    control_cfg::Any
    cfg::SimplePendulumMPPIConfig
...
function build_simple_pendulum_certifier(
    ...
    return SimplePendulumScalingCertifier(
        problem,
        system_cfg,
        control_cfg,
        cfg,
...
function SC.set_problem!(
    cert::SimplePendulumScalingCertifier,
...
function SC.set_trajectory!(cert::SimplePendulumScalingCertifier, candidate)
...
function SC.certify!(cert::SimplePendulumScalingCertifier)
    state_scaling = resolve_state_scaling(cert.cfg, cert.candidate)
    cert.inner = _build_simple_pendulum_certifier(
        cert.problem,
        cert.system_cfg,
        cert.control_cfg,
        cert.cfg,
...
SC.get_result(cert::SimplePendulumScalingCertifier) = cert.result
SC.get_success(cert::SimplePendulumScalingCertifier) = cert.success
SC.get_solve_time(cert::SimplePendulumScalingCertifier) = cert.solve_time_sec

function active_state_scaling(cert::SimplePendulumScalingCertifier)
    cert.inner === nothing && return nothing
    return cert.inner.config.options.state_scaling
end

function certifier_state_scaling(cfg::SimplePendulumMPPIConfig)
    cfg.state_scaling_mode == :matrix &&
        return matrix_state_scaling(cfg.state_scaling_matrix)
    cfg.state_scaling_mode == :legacy &&
        return cfg.use_state_scaling ? Float64.(cfg.state_scaling) : nothing
    cfg.state_scaling_mode == :none && return nothing
    cfg.state_scaling_mode == :std && return "trajectory_std"
    return "unknown($(cfg.state_scaling_mode))"
end
...
function simple_pendulum_timing_summary(run_result)
...
function print_simple_pendulum_timing_summary(timings)
    println("\n=== Simple pendulum MPPI timing summary ===")
...
function save_simple_pendulum_timing_summary(path::AbstractString, timings)
    open(path, "w") do io
...
function print_certification_diagnostics(run_result, cfg, terminal_data)
    ...
    if cert.lmi_data !== nothing &&
       hasproperty(cert.lmi_data, :ellipsoids) &&
       !isempty(cert.lmi_data.ellipsoids)
        println(
            "terminal ellipsoid volume: ",
            _ellipsoid_volume_or_missing(first(cert.lmi_data.ellipsoids)),
        )
        println(
            "initial certified ellipsoid volume: ",
            _ellipsoid_volume_or_missing(last(cert.lmi_data.ellipsoids)),
        )
    end
    println(
    ...
    println("state scaling mode: ", cfg.state_scaling_mode)
    println("state scaling used by certifier: ", active_scaling)
...
function main(
    ...
    timings = simple_pendulum_timing_summary(run_result)
    print_simple_pendulum_timing_summary(timings)
    timing_csv = save_simple_pendulum_timing_summary(
...
function run_simple_pendulum_certification_mode(; adaptive_linearization_boxes::Bool)
    cfg = SimplePendulumMPPIConfig(;
        ...
        plot_gif = false,
        plot_mp4 = false,
        kappa_statistical_samples = 10000,
    )
    run_result = main(cfg)
    println(
