...
Base.@kwdef struct MarcheArriereConfig
    Δt::Float64 = 0.2
    hx::SVector{4, Float64} = SVector(0.4, 0.4, 4 * (pi / 180), 4 * (pi / 180))
    ...
    state_scaling_matrix::Matrix{Float64} =
        Matrix{Float64}(LA.Diagonal([17.0, 11.0, 2pi, deg2rad(55.0)] .* 1.5))
    ...
    ΔX_max::Vector{Float64} = [27.0, 21.0, 2pi, 2deg2rad(55.0)]
    ΔU_initial::Vector{Float64} = [0.10, 0.10]
    ΔU_min::Vector{Float64} = [1.0e-4, 1.0e-4]
    ΔU_max::Vector{Float64} = [20.0, 13.0]
    ...
    adaptive_box_search_scales::Vector{Float64} =
        [0.7, 0.85, 1.0, 1.2, 1.5, 2.0, 3.0, 5.0, 7.0, 10.0]
    ...
    mppi_noise_v::Float64 = 0.55
    mppi_noise_σ::Float64 = 0.25
    ...
    kappa_statistical_samples::Int = 100
    ...
    empirical_inflation_shell_sampling::Bool = true
    empirical_inflation_seed::Int = 2
    empirical_inflation_rollout_alpha::Float64 = 2.0
    empirical_inflation_save_rollout_plot::Bool = true
end
...
function terminal_john_ellipsoid_data(problem, cfg::MarcheArriereConfig)
...
function terminal_ellipsoidal_distance2(
    ...
    e = periodic_state_error(x, terminal_center, cfg)
    return Float64(collect(e)' * terminal_shape * collect(e))
end
...
function matrix_state_scaling(M)
    ...
    return [10.0, 10.0, deg2rad(55.0), deg2rad(55.0)]*2.2
end # [2.0, 1.5, 0.35, 0.35] # [0.75, 0.60, 0.25, 0.25] #[10.0, 10.0, deg2rad(55.0), deg2rad(55.0)] 

function resolve_state_scaling(cfg::MarcheArriereConfig, candidate)
...
function build_control_problem()
    ...
    target_set = UT.HyperRectangle(
        SVector(14.0, 4.5, pi - 20 * (pi / 180), -20 * (pi / 180)),
        SVector(16.0, 6.9, pi + 20 * (pi / 180), 20 * (pi / 180)),
    )
...
function _build_marche_arriere_mppi_certifier(
    problem,
    system_cfg,
    cfg::MarcheArriereConfig;
    vehicle_module,
...
mutable struct MarcheArriereScalingCertifier <: SC.AbstractSymbolicCertifier
    problem::Any
    system_cfg::Any
    cfg::MarcheArriereConfig
    vehicle_module::Any
...
function build_marche_arriere_mppi_certifier(
    problem,
    system_cfg,
    cfg::MarcheArriereConfig;
    vehicle_module,
    ...
    return MarcheArriereScalingCertifier(
        problem,
        system_cfg,
        cfg,
        vehicle_module,
...
function SC.set_problem!(
    cert::MarcheArriereScalingCertifier,
...
function SC.set_trajectory!(cert::MarcheArriereScalingCertifier, candidate)
...
function SC.certify!(cert::MarcheArriereScalingCertifier)
    state_scaling = resolve_state_scaling(cert.cfg, cert.candidate)
    cert.inner = _build_marche_arriere_mppi_certifier(
        cert.problem,
        cert.system_cfg,
        cert.cfg;
    ...
    if cert.success && cert.cfg.strict_reach_avoid_audit
        audit = audit_reach_avoid_ellipsoids(
            cert.problem,
            cert.system_cfg,
            cert.result,
            cert.cfg,
...
SC.get_result(cert::MarcheArriereScalingCertifier) = cert.result
SC.get_success(cert::MarcheArriereScalingCertifier) = cert.success
SC.get_solve_time(cert::MarcheArriereScalingCertifier) = cert.solve_time_sec
...
function marche_arriere_timing_summary(run_result)
...
function print_marche_arriere_timing_summary(timings)
    println("\n=== Marche arriere timing summary ===")
...
function save_marche_arriere_timing_summary(path::AbstractString, timings)
    open(path, "w") do io
...
function active_state_scaling(cert::MarcheArriereScalingCertifier)
...
function _periodic_full_domain_dims(rect, cfg::MarcheArriereConfig; tol::Float64 = 1.0e-8)
    dims = Int[]
    for i in eachindex(cfg.periodic_dims)
        d = Int(cfg.periodic_dims[i])
        width = Float64(rect.ub[d] - rect.lb[d])
        if abs(width - Float64(cfg.periodic_periods[i])) <= tol
            push!(dims, d)
        end
    end
    return dims
...
function ellipsoid_inside_hyperrectangle(
    E,
    rect,
    cfg::MarcheArriereConfig;
    tol::Float64 = 1.0e-8,
    ...
    ignored_dims = Set(_periodic_full_domain_dims(rect, cfg; tol = tol))
    checked_dims = [i for i in 1:n if !(i in ignored_dims)]
    isempty(checked_dims) && return true, 0.0

    lower_violation = maximum(Float64(rect.lb[i]) - lo[i] for i in checked_dims)
    upper_violation = maximum(hi[i] - Float64(rect.ub[i]) for i in checked_dims)
...
function audit_reach_avoid_ellipsoids(
    ...
    for (i, E) in enumerate(ellipsoids)
        ...
        inside_domain, domain_violation =
            ellipsoid_inside_hyperrectangle(E, Xbase, cfg; tol = tol)
        if !inside_domain
            push!(
                violations,
                (;
...
function main(cfg::MarcheArriereConfig = MarcheArriereConfig())
    run_result = run_vehicle_benchmark(
        ...
        certifier_builder = build_marche_arriere_mppi_certifier,
    )
    timings = marche_arriere_timing_summary(run_result)
    print_marche_arriere_timing_summary(timings)
    timing_csv = save_marche_arriere_timing_summary(
