...
function build_backend(; verbose::Bool = false)
    return optimizer_with_attributes(
        ...
        # MOI.RawOptimizerAttribute("MSK_IPAR_PRESOLVE_USE") => "MSK_PRESOLVE_MODE_ON",
        # MOI.RawOptimizerAttribute("MSK_IPAR_LOG_PRESOLVE") => 10,

        # MOI.RawOptimizerAttribute("MSK_IPAR_INTPNT_SOLVE_FORM") => "MSK_SOLVE_DUAL",
        # MOI.RawOptimizerAttribute("MSK_IPAR_LOG_INTPNT")        => 10,
    )
end
...
function build_control_problem()
    x0 = SVector(0.5, 4.0, 0.0, 0.0, 0.0)

    initial_set = UT.HyperRectangle(
        SVector(0.0, 3.45, -deg2rad(6.0), -deg2rad(6.0), -deg2rad(6.0)),
        SVector(1.0, 4.55,  deg2rad(6.0),  deg2rad(6.0),  deg2rad(6.0)),
    )

    target_set = UT.HyperRectangle(
        SVector(7.0, 3.0, -deg2rad(8.0), -deg2rad(8.0), -deg2rad(8.0)),
        SVector(8.0, 5.0,  deg2rad(8.0),  deg2rad(8.0),  deg2rad(8.0)),
    )
...
function main(cfg::MarcheArriereConfig = MarcheArriereConfig())
    ...
    save_state_space_plots!(
        ...
        periodic_periods = cfg.periodic_periods,
        periodic_start = cfg.periodic_start,
        title12 = "marche_avant (x,y)",
        title34 = "marche_avant (theta,phi)",
    )
