...
function _initTargetSets()
    ...
    _T_ = UT.HyperRectangle(SVector(3., 0.3, -100.0), SVector(3.6, 0.8, 100.0))
    return _I_, _T_
end
...
function get_obstacles(_X_;
    ...
    return [UT.HyperRectangle(SVector(x1lb, x2lb, _X_.lb[3]), SVector(x1ub, x2ub, _X_.ub[3])) for (x1lb, x2lb, x1ub, x2ub) in zip(X1_lb, X2_lb, X1_ub, X2_ub)]
end
...
function problem(;simple=false, approx_mode="growth")
    if simple
        ...
        _X_ = UT.HyperRectangle(SVector(0.0, 0.0, -pi-0.4), SVector(10., 10.0, pi+0.4))
        _I_ = UT.HyperRectangle(SVector(0.4, 0.4, 0.0), SVector(0.4, 0.4, 0.0))
        _T_ = UT.HyperRectangle(SVector(9., 0.3, -100.0), SVector(9.6, 0.8, 100.0))
    end
