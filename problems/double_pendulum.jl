...
function safety_problem(;
    ...
    if objective == "safety_down"
        _X_ = UT.HyperRectangle(SVector(-π/4.0, -π/4.0, -5.0, -5.0), SVector(π/4.0, π/4.0, 5.0, 5.0))
        _U_ = UT.HyperRectangle(SVector(-2.5), SVector(2.5))
        _I_ = UT.HyperRectangle(SVector(-3.0*pi/180.0, -3.0*pi/180.0, -0.5, -0.5), SVector(3.0*pi/180.0, 3.0*pi/180.0, 0.5, 0.5))
        _S_ = UT.HyperRectangle(SVector(-35.0*pi/180.0, -35.0*pi/180.0, -1.0, -1.0), SVector(35.0*pi/180.0, 35.0*pi/180.0, 1.0, 1.0))
    end
...
function optimal_control_problem(;l1 = 1.0, l2 = 1.0, m1 = 1.0, m2 = 1.0, g = 9.81)
    ...
    _U_ = UT.LazySetMinus(
        ...
        _I_ = UT.HyperRectangle(SVector(-3.0*pi/180.0, -3.0*pi/180.0, -0.5, -0.5), SVector(3.0*pi/180.0, 3.0*pi/180.0, 0.5, 0.5)),
    _T_ = UT.HyperRectangle(SVector(35.0*pi/180.0, -15.0*pi/180.0, -1.0, -1.0), SVector(45.0*pi/180.0, 15.0*pi/180.0, 1.0, 1.0)),
    )
    ...
    _I_ = UT.HyperRectangle(SVector(-3.0*pi/180.0, -3.0*pi/180.0, -0.5, -0.5), SVector(3.0*pi/180.0, 3.0*pi/180.0, 0.5, 0.5))
    _T_ = UT.HyperRectangle(SVector(-π/2.0, pi-50.0*pi/180.0, -4.5, -4.5), SVector(π/2.0, pi+50.0*pi/180.0, 4.5, 4.5))
    sys = system(; l1 = l1, l2 = l2, m1 = m1, m2 = m2, g = g, _X_ = _X_, _U_ = _U_)
