...
function system(;
    ...
    _X_ = LazySets.Hyperrectangle(;
        low = SVector(-π, -π, -5.0, -5.0),
        high = SVector(π, π, 5.0, 5.0),
    ),
    _U_ = LazySets.Hyperrectangle(; low = SVector(-11.0), high = SVector(11.0)),
)
...
function safety_problem(; params::Params = Params(), objective = "safety_down")
    if objective == "safety_down"
        _X_ = LazySets.Hyperrectangle(;
            low = SVector(-π / 4.0, -π / 4.0, -5.0, -5.0),
            high = SVector(π / 4.0, π / 4.0, 5.0, 5.0),
        )

        _U_ = LazySets.Hyperrectangle(; low = SVector(-2.5), high = SVector(2.5))

        _I_ = LazySets.Hyperrectangle(;
            low = SVector(-3.0π / 180.0, -3.0π / 180.0, -0.5, -0.5),
            high = SVector(3.0π / 180.0, 3.0π / 180.0, 0.5, 0.5),
        )

        _S_ = LazySets.Hyperrectangle(;
            low = SVector(-35.0π / 180.0, -35.0π / 180.0, -1.0, -1.0),
            high = SVector(35.0π / 180.0, 35.0π / 180.0, 1.0, 1.0),
        )
    else
...
function optimal_control_problem(;
    ...
    if objective == "swing_up"
        _X_ = LazySets.Hyperrectangle(;
            low = SVector(-π / 2.0, -π, -5.0, -5.0),
            high = SVector(π / 2.0, π, 5.0, 5.0),
        )

        _U_ = UT.set_minus(
            LazySets.Hyperrectangle(; low = SVector(-5.5), high = SVector(5.5)),
        ...
        _T_ = LazySets.Hyperrectangle(;
            low = SVector(-π / 2.0, π - 50.0π / 180.0, -4.5, -4.5),
            high = SVector(π / 2.0, π + 50.0π / 180.0, 4.5, 4.5),
        )
    else
        error("Unknown objective: $objective")
    end
