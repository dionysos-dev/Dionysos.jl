...
function problem(; objective = "reachability-up-low_power")
    ...
    elseif objective == "safety-down"
        ...
        _S_ = UT.HyperRectangle(SVector(-8.0*pi/180.0, -1.0), SVector(8.0*pi/180.0, 1.0))
        return PB.SafetyProblem(sys, _I_, _S_, PB.Infinity())
    elseif objective == "reachability-up-ultra_low_power"
        sys = system(_X_ = UT.HyperRectangle(SVector(-π, -10.0), SVector(π+pi, 10.0)), _U_ = UT.HyperRectangle(SVector(-3.0), SVector(3.0)))
        _I_ = UT.HyperRectangle(SVector(-5.0*pi/180.0, -0.5), SVector(5.0*pi/180.0, 0.5))
        _T_ = UT.HyperRectangle(SVector(pi-5.0*pi/180.0, -1.0), SVector(pi+5.0*pi/180.0, 1.0))
    ...
    elseif objective == "reachability-up-high_power"
        ...
        _I_ = UT.HyperRectangle(SVector(-5.0*pi/180.0, -0.5), SVector(5.0*pi/180.0, 0.5))
        _T_ = UT.HyperRectangle(SVector(pi-5.0*pi/180.0, -2.0), SVector(pi+5.0*pi/180.0, 2.0))
        return PB.OptimalControlProblem(sys, _I_, _T_, nothing, nothing, PB.Infinity())
    end
end
